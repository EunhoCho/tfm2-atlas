param(
    ...
    [string]$SdkDir = (Join-Path $PSScriptRoot "..\.build\sdk-0.5.5\mod-sdk")
)
...
if ($pinned) {
    $env:RUSTUP_TOOLCHAIN = "$pinned-x86_64-pc-windows-msvc"
...
cargo test --manifest-path $manifest --target-dir $targetDir --lib
if ($LASTEXITCODE -ne 0) { exit $LASTEXITCODE }
...
$outDll = Join-Path $modRoot "tfm2_atlas_client_055.dll"
